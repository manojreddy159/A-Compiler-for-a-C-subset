...
void func3(ofstream & fp,string s1,string s2,string s3,string s4){
	...
	if((!isNumber(s3)&&s3[s3.length()-1]=='1')||(isNumber(s3)&&!isfloat(s3))){
      ...
      if(s1=="!="){fp<<"c.eq.s ";fp<< "$f2, $f4"<<endl;fp<<"bc1t P"<<z<<endl;}
        if(s4[s4.length()-1]=='1'){fp<<"li $t8,1"<<endl<<"sw $t8,"+s4<<endl<<"j P"<<z+1<<endl;z=z+2;
        fp<<"P"<<z-2<<":"<<endl<<"li $t8,0"<<endl<<"sw $t8,"+s4<<endl<<"P"<<z-1<<":\n";}
        else{fp<<"li.s $f8,1.0"<<endl<<"s.s $f8,"+s4<<endl<<"j P"<<z+1<<endl;z=z+2;
        fp<<"P"<<z-2<<":"<<endl<<"li.s $f8,0.0"<<endl<<"s.s $f8,"+s4<<endl<<"P"<<z-1<<":\n";}
  ...
  if (myfile.is_open())
  {
    while ( getline (myfile,line) )
    {
      ...
      if(s1=="+"||s1=="-"||s1=="*"||s1=="/"||s1=="%"){func(fp,s1,s2,s3,s4);}
       ...
       if(s1=="if"){
        if(!isNumber(s2)&&s2[s2.length()-1]=='1'){fp<<"lw $t0,"+s2<<endl<<"li $t2,1"<<endl<<"bge $t0,$t2,"+s4<<endl;}
        else if(!isNumber(s2)&&s2[s2.length()-1]=='2'){fp<<"l.s $f2,"+s2<<endl;fp<<"cvt.w.s $f2, $f2"<<endl<<"mfc1 $t0,$f2"<<endl<<"li $t2,1"<<endl<<"bge $t0,$t2,"+s4<<endl;}
        else if(isNumber(s2)&&!isfloat(s2)){fp<<"li $t0,"+s2<<endl<<"li $t2,1"<<endl<<"bge $t0,$t2,"+s4<<endl;}
        else{fp<<"li.s $f2,"+s2<<endl<<"cvt.w.s $f2, $f2"<<endl<<"mfc1 $t0,$f2"<<endl<<"li $t2,1"<<endl<<"bge $t0,$t2,"+s4<<endl;}
       }
       if(s1=="cnvrt_float"){
                              if(!isNumber(s2)){
                                string a1,a2;
                              ...
                              if(pos==string::npos){fp<<"lw $t0,"+s2<<endl<<"mtc1 $t0,$f2"<<endl<<"cvt.s.w $f2, $f2"<<endl<<"s.s $f2,"+s4<<endl;}
                                ...
                                fp<<"li $t0,"+s2<<endl<<"mtc1 $t0,$f2"<<endl<<"cvt.s.w $f2, $f2"<<endl<<"s.s $f2,"+s4<<endl;
                              }
